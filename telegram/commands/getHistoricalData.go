...
func getHistoricalData(bot *telegramBot.BotAPI, update telegramBot.Update, instrument string, fromDate string, toDate string, resolution string) {
	bot.Send(telegramUtils.GenerateReplyMessage(update, "Fetching Historical Data"))

	from, _ := time.Parse(time.RFC3339, fromDate)
	to, _ := time.Parse(time.RFC3339, toDate)

	data, err := fyersSDK.FetchHistoricalData(instrument, resolution, from.Unix(), to.Unix(), 0)
	if err != nil {
